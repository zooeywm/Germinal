...
GNativeProtocol 用于显式请求模式切换，例如：

- enter-native-app-mode
- update-native-app-ui
- exit-native-app-mode

GShell 不通过智能识别判断一个进程是否是 GNativeApp，而是依赖 GNativeProtocol 的明确消息。

## GNativeAppMode

GShell 的结构化应用模式。

GNativeAppMode 由 GNativeApp 通过 GNativeProtocol 显式进入。
...
GNativeApp 通过 GNativeProtocol 请求进入和退出 GNativeAppMode。
...
在本地模式下，推荐由 GNativeApp 先通过 PTY / GNativeProtocol 显式请求进入 GNativeAppMode，再建立专用 transport 发送 RenderCommand 帧。
