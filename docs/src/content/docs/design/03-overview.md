...
Germinal，全称是 Graphical Terminal，是在传统终端交互范式基础上，以结构化 UI 为主、以传统终端兼容为辅的键盘优先、命令驱动图形化终端系统。
...
3. 通过 GShellProtocol 支持从 PtyMode 显式进入 GNativeAppMode，而不是依赖智能识别。
...
Germinal 不通过智能识别判断一个进程是否是 GNativeApp。GNativeAppMode 的进入和退出必须由明确的 GShellProtocol 触发。

Germinal 第一版不追求完整兼容所有历史终端行为，只保证传统 Shell、CLI 和常见 TUI 程序能够通过 PtyHost 正常运行。
...
1. 作为传统终端兼容层，通过 PtyHost 承载 Shell、CLI 和现有 TUI 程序。
2. 作为结构化应用平台，通过 GShellProtocol 让支持 Germinal 的应用进入 GNativeAppMode，承载具备自由布局、GPU 渲染和统一交互模型的新型开发者应用。
...
1. 可以稳定运行传统 Shell、CLI 和常见 TUI 程序，并通过 PtyHost 保持基本终端兼容能力。
2. 每个 Pane 可以稳定承载一个 GShell，GShell 默认进入 PtyMode。
3. 可以通过明确的 GShellProtocol 从 PtyMode 进入 GNativeAppMode，并在 GNativeApp 退出后返回 PtyMode。
