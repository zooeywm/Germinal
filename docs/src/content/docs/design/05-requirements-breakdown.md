...
模式切换必须由 GNativeProtocol 显式触发，不做智能识别。
...
系统不通过进程名、输出内容或启发式规则判断 GNativeApp。模式切换必须由 GNativeProtocol 明确触发。
...
#### R5.2 GNativeProtocol

GNativeApp 需要通过 GNativeProtocol 显式请求进入、更新和退出 GNativeAppMode。
...
#### R5.9 RenderCommand Transport

GNativeApp 需要把 RenderCommand 通过独立 transport 发送给 GShell。

推荐约束如下：

- PTY 只负责启动入口和 `enter-native-app-mode` / `exit-native-app-mode` 这类模式切换握手。
- RenderCommand 不应长期混在 PTY 文本流中传输。
- 命令通道应优先承载小而频繁的结构化帧消息。
- 图片、视频帧、离屏 surface 等大资源不应反复内嵌在 RenderCommand 中，而应通过 `resource_id` 引用独立资源通道。
- 第一版本地实现优先推荐 `Unix domain socket` 作为命令通道；未来可在资源通道中补充 `shared memory / memfd`。
...
推荐形态是帧级 envelope 加命令级 `opcode + payload` 二进制记录，而不是直接暴露底层 GPU 命令对象。
